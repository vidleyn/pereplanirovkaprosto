...
// Selected item for placement on 2D floorplan
...
class ViewManager {
  ...
  showFloorPlan() {
    ...
    hide($('#viewcontrols'));
    // Wireframe button is outside viewcontrols, so it stays visible
...
function init() {
  ...
  // Setup item adding - support both 3D and 2D placement
  // Items can be added by clicking on room in 2D view after selecting from modal
  // Modal can be opened via keyboard shortcut or other method if needed
  ...
  if (addItems) {
    addItems.addEventListener('mousedown', (e) => {
      ...
      // Store selected item for 2D placement
      ...
      // If in 3D view and wall/floor is selected, place immediately
      if (viewManager.designView && hasClass(viewManager.designView, 'active')) {
        ...
        // In 2D view - close modal and wait for room click
        modalManager.close();
        // Show hint
        const hint = $('#draw-walls-hint');
        if (hint) {
          hint.textContent = `Click on a room to place "${metadata.itemName}"`;
          addClass(hint, 'show');
        }
      }
    });
  ...
  // Handle room clicks in 2D view to place items
  blueprint3d.model.floorplan.addEventListener(BP3DJS.EVENT_ROOM_2D_CLICKED, (event) => {
    if (selectedItemForPlacement) {
      const room = event.item;
      if (room && room.areaCenter) {
        // Get room center position (areaCenter is Vector2 with x, y in cm)
        const center = room.areaCenter;
        // Convert 2D coordinates to 3D (x, y in cm -> x, 0, y for 3D)
        // Create position object - addItem accepts object with x, y, z or Vector3
        const position = { x: center.x, y: 0, z: center.y };
        
        // Add item to the scene at room center
        blueprint3d.model.scene.addItem(
          selectedItemForPlacement.itemType,
          selectedItemForPlacement.modelUrl,
          selectedItemForPlacement.metadata,
          position,
          null,
          null,
          false,
