...
export const wallColorBearing = "#ff0000"; // Красный цвет для несущих стен

export const edgeColor = "#888888";
export const edgeColorHover = "#008cba";
export const edgeWidth = 1;

export const deleteColor = "#008cba";
...
export class FloorplannerView2D {
  ...
  draw() {
    ...
    if (this.viewmodel.mode == floorplannerModes.DRAW) {
      ...
      if (
        ...
        this.context.strokeStyle = "#008cba";
  ...
  drawWall(wall) {
    ...
    // Несущие стены отображаются красным цветом (если не выбраны и не в режиме удаления)
    if (
      wall.isBearing &&
      !selected &&
      !hover &&
      this.viewmodel.mode != floorplannerModes.DELETE
    ) {
      color = wallColorBearing;
    } else if (hover && this.viewmodel.mode == floorplannerModes.DELETE) {
      color = deleteColor;
    } else if (hover) {
    ...
    if (wall.wallType == WallTypes.STRAIGHT) {
      ...
      //			this.drawBezierObject(wall.bezier, 10, '#008cba');
      ...
      //			this.drawLine(this.viewmodel.convertX(project.x),this.viewmodel.convertY(project.y),this.viewmodel.convertX(p.x),this.viewmodel.convertY(p.y), 1, '#008cba');
    }
  ...
  drawCurvedLine(startX, startY, aX, aY, bX, bY, endX, endY, width, color) {
    ...
    // width is an integer
    // color is a hex string, i.e. #008cba
    this.context.beginPath();
    this.context.moveTo(startX, startY);
    this.context.bezierCurveTo(aX, aY, bX, bY, endX, endY);
    //		this.context.closePath();
  ...
  drawPolygonCurved(
    ...
    // fillColor is a hex string, i.e. #008cba
    fill = fill || false;
    stroke = stroke || false;
    this.context.beginPath();
    ...
    //			{
    //				this.drawCircle(this.viewmodel.convertX(pointset[0].x), this.viewmodel.convertY(pointset[0].y), 5, '#008cba');
